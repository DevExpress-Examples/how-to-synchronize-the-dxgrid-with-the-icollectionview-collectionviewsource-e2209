	...
	Public Class GridControlDataModel
		...
		Private Sub ConnectToGridControl(ByVal gridControl As GridControl)
			...
			Try
				...
				SyncFocusedRowHandle()
				SyncFilter()
				gridControl.View.AllowGrouping = collectionView_Renamed.CanGroup
			...
			If CollectionView.SortDescriptions IsNot Nothing Then
				...
				Try
					...
					For Each sortDescription As SortDescription In CollectionView.SortDescriptions
						gridControl.SortBy(gridControl.Columns(sortDescription.PropertyName),If(sortDescription.Direction = ListSortDirection.Ascending, ColumnSortOrder.Ascending, ColumnSortOrder.Descending))
		...
		Private Sub SyncGrouping()
			If syncGroupSortLocker.IsLocked Then
				Return
			End If
			syncGroupSortLocker.DoLockedAction(Function() AnonymousMethod2())
		End Sub
			...
			If CollectionView.GroupDescriptions IsNot Nothing Then
				...
				Try
					...
					For Each groupDescription As PropertyGroupDescription In CollectionView.GroupDescriptions
						...
						Dim sortDirection As ListSortDirection = If(sortGroupDescription IsNot Nothing, sortGroupDescription.SortDirection, ListSortDirection.Ascending)
						gridControl.GroupBy(gridControl.Columns(groupDescription.PropertyName),If(sortDirection = ListSortDirection.Ascending, ColumnSortOrder.Ascending, ColumnSortOrder.Descending))
